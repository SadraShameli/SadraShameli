<!--
      ...
      ╚██╗       hey, you're reading the source. i like you already.
       ╚██╗
       ██╔╝      every graphic on this page is generated by ~1.8k lines
      ██╔╝       of stdlib python in Scripts/readme. the live cards are rebuilt
      ╚═╝        every hour by .github/workflows/readme.yml.
           ███████╗
...
- **This README is a build artifact.** The SVGs are generated by [`Scripts/readme`](Scripts/readme) using only the Python standard library, with fonts subset and embedded so they render the same everywhere. No third-party stats services, so nothing can rate-limit it.
...
This card is rebuilt every hour from sadra.nl's public API. When the unit on my desk is plugged in it says **LIVE**. When it isn't, it tells you so.

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/SadraShameli/SadraShameli/output/live-sensors-dark.svg">
  <img alt="Latest SensorHub readings from Rijswijk: temperature, humidity, air pressure and loudness" src="https://raw.githubusercontent.com/SadraShameli/SadraShameli/output/live-sensors-light.svg" width="100%">
</picture>

<details>
<summary><b>How a reading gets from my desk to this page</b></summary>
<br>

```mermaid
flowchart LR
    subgraph unit["SensorHub unit · ESP32 + FreeRTOS"]
        bme["BME680<br/>temp · humidity · pressure · gas"] --> climate(["Climate task"])
        mic["INMP441 I²S mic"] --> mictask(["Mic task"])
        climate --> reg[("Sensor registry<br/>atomic min / max")]
        mictask --> reg
        mictask -- "too loud" --> adpcm(["IMA-ADPCM encoder<br/>5 s pre-roll + 10 s post-roll"])
        reg --> net(["Network task"])
    end
    subgraph site["sadra.nl · Next.js 16"]
        api["tRPC + Zod"] --> db[("PostgreSQL<br/>via Drizzle")]
        db --> ui["Live readings<br/>+ noise recordings"]
    end
    net -- "HTTPS · POST /api/reading" --> api
    adpcm -- "streamed WAV · POST /api/recording" --> api
    db -. "hourly GitHub Action" .-> readme["the card above ↑"]
```
...
My last 12 months on GitHub, played back as an audio waveform. Each bar is a day, and the ▼ marks the loudest one.

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/SadraShameli/SadraShameli/output/live-contributions-dark.svg">
  <img alt="GitHub contributions over the last 12 months drawn as an audio waveform with a moving playhead" src="https://raw.githubusercontent.com/SadraShameli/SadraShameli/output/live-contributions-light.svg" width="100%">
</picture>

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/SadraShameli/SadraShameli/output/live-stats-dark.svg">
  <img alt="GitHub stats: contributions, current and longest streak, repositories, stars and languages" src="https://raw.githubusercontent.com/SadraShameli/SadraShameli/output/live-stats-light.svg" width="100%">
</picture>
...
```mermaid
%%{init: {"gitGraph": {"mainBranchName": "life", "rotateCommitLabel": true}} }%%
gitGraph
    commit id: "hello, world"
    branch build
    commit id: "2020 · arduino robot"
    commit id: "social robot"
    checkout life
    branch work
    commit id: "2021 · blue star planning"
    checkout build
    commit id: "2022 · project a.i."
    checkout life
    merge build id: "95-page A.I. thesis"
    checkout work
    commit id: "iot fleet + azure ci/cd"
    checkout life
    branch trade
    commit id: "~2023 · trading NQ futures"
    checkout build
    commit id: "2024 · sensorhub"
    commit id: "2024 · sadra.nl"
    checkout work
    commit id: "2024 · nobears"
    commit id: "2024 · mino markt"
    checkout trade
    commit id: "prop firm payouts"
    commit id: "2025 · tradingbot live"
    checkout life
    merge work id: "4+ yrs in production"
    merge build id: "ship it"
    merge trade id: "2026 · you are here" type: HIGHLIGHT
```
...
<p align="center"><sub>Graphics generated by <a href="Scripts/readme"><code>Scripts/readme</code></a> · live cards rebuilt hourly by <a href=".github/workflows/readme.yml">a GitHub Action</a> · fonts: Geist & Orbitron (SIL OFL)</sub></p>
