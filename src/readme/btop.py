...
def _box(t: Theme, x: float, y: float, w: float, h: float, title: str, right: str = "") -> str:
...
def _load(i: int, n: int) -> float:
    ...
    a = 2 * math.pi * i / n
    v = 0.5 + 0.2 * math.sin(2 * a) + 0.14 * math.sin(5 * a + 1.3) + 0.09 * math.sin(11 * a + 0.4)
    return min(max(v, 0.08), 0.98)


def _cpu(t: Theme, term: Terminal, x: float, y: float, w: float, h: float) -> str:
    sp, r_on, r_off = 9, 2.7, 1.4
    gx, gy = x + 18, y + 20
    cols, rows = int((w - 36) // sp), int((h - 34) // sp)
    gw, gh = cols * sp, rows * sp
    colors = [t.green if f <= 0.5 else t.yellow if f <= 0.8 else t.red for f in ((r + 1) / rows for r in range(rows))]
    dots = []
    for c in range(cols * 2):  # two periods, so shifting by one period loops seamlessly
        lit = round(_load(c % cols, cols) * rows)
        cx = gx + c * sp + sp / 2
        for r in range(lit):
            dots.append(f'<circle cx="{cx:.1f}" cy="{gy + gh - r * sp - sp / 2:.1f}" r="{r_on}" fill="{colors[r]}"/>')
    term.css.append(
        f"@keyframes scroll{{to{{transform:translateX(-{gw}px)}}}}.scroll{{animation:scroll 18s linear infinite}}"
    )
    return (
        _box(t, x, y, w, h, "cpu", "uptime 4+ yrs")
        + "<defs>"
...
def _mem(t: Theme, term: Terminal, x: float, y: float, w: float, h: float) -> str:
    tx, bx, bw = x + 18, x + 118, w - 118 - 66
    parts = [_box(t, x, y, w, h, "mem", "of the human kind"), f"<defs>{_meter_gradient(t, 'mg', bx, bw)}</defs>"]
...
def _proc(t: Theme, term: Terminal, x: float, y: float, w: float, h: float) -> str:
    ...
    frames = "".join(
        f"{pct(i * 1.8, period)}{{transform:translateY({i * rh}px)}}" for i in range(len(PROCS))
    )
    term.css.append(
    ...
    head_y = y + 24
    for label, hx, anchor in (("pid", col_pid, "start"), ("program", col_prog, "start"),
                              ("arguments", col_args, "start"), ("cpu%", x + w - 18, "end")):
        parts.append(
    ...
    for i, (pid, prog, args, cpu) in enumerate(PROCS):
        ...
        parts.append(
            ...
            + (f'<tspan fill="{t.red}" dx="12">starved</tspan>' if starved else "")
            + "</text>"
...
def btop(t: Theme) -> str:
    ...
    term.body.append(f'<g class="{term.appear(done + 0.25, 0.4)}">{panels}</g>')

    prompt_y = proc_y + proc_h + 40
    cx = term.prompt(X0, prompt_y, "~")
    term.cursor(cx, prompt_y, done + 0.8)
    desc = (
        "btop --user sadra. mem: " + ", ".join(f"{k} {v}%" for k, v in MEM) + ". Running: "
        + ", ".join(f"{prog} {args} ({cpu}% cpu)" for _, prog, args, cpu in PROCS) + "."
    )
