...
def _typing(t: Theme) -> tuple[str, str]:
    ...
    cmd = "whoami"
    chars: list[
        tuple[str, float, float, int, int, str]
    ] = []  # ch, on, off, line, col, cls
    ...
    for n, (ch, on, off, line, col, cls) in enumerate(chars):
        ...
        els.append(
            f'<text id="c{n}" class="{cls}" x="{x:.2f}" y="{y}"{style}>{esc(ch)}</text>'
        )
    ...
    y = TY2 + 6
    wave = "".join(
        f"q{CW / 4:.2f} {-3 if k % 2 else 3} {CW / 2:.2f} 0" for k in range(12)
    )
    squiggle = (
...
def _iso(
    ox: float, oy: float, x: float, y: float, z: float
) -> tuple[float, float]:
...
def _pts(ox, oy, *xyz) -> str:
    return " ".join(
        f"{px:.2f},{py:.2f}" for px, py in (_iso(ox, oy, *p) for p in xyz)
    )
