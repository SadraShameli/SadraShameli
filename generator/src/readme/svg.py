...
def measure(
    text: str, kind: str, weight: int, size: float, spacing: float = 0.0
) -> float:
...
def document(
    w: float, h: float, body: str, css: str, title: str, desc: str = ""
) -> str:
...
def card_frame(
    t: Theme, w: float, h: float, radius: int = 16, open_bottom: bool = False
) -> str:
...
def clip_card(
    cid: str, w: float, h: float, radius: int = 16, open_bottom: bool = False
) -> str:
    extra = (
...
def pct(t: float, total: float) -> str:
    return (
        f"{min(max(t / total * 100, 0), 100):.3f}".rstrip("0").rstrip(".")
        + "%"
    )
