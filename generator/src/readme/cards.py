...
Photos are pre-cropped by `uv run readme photos` and embedded as data URIs so each
...
def plus(
    x: float, y: float, color: str, size: float = 6, opacity: float = 0.8
) -> str:
...
def wrap(
    text: str, kind: str, weight: int, size: float, max_w: float
) -> list[str]:
    lines, cur = [], ""
...
def _lidar(t: Theme, cx: float, cy: float) -> tuple[str, str]:
    ...
    for i, (dx, dy) in enumerate(pts):
        hits.append(
            f'<circle class="hit h{i}" cx="{cx + dx}" cy="{cy + dy}" r="3" fill="#4ade80"/>'
        )
...
TRADE_LOG = [
    ...
    (
        "14:32:00",
        "check",
        [("news window ", "dim"), ("fail", "bad"), (" → skip", "bad")],
    ),
...
def _tradelog(
    t: Theme, x: float, y: float, w: float, h: float
) -> tuple[str, str]:
    ...
    css.append(
        "@keyframes live{0%,100%{opacity:1}50%{opacity:.25}}#live{animation:live 1.6s ease-in-out infinite}"
    )
    ...
    for n in range(len(TRADE_LOG) * 2):
        ...
        tag_w = 5 * cw + 10
        tspans = "".join(
            f'<tspan fill="{tone_color[tone]}">{esc(text)}</tspan>'
...
def _chip(
    ...
    dot_el = (
        f'<circle cx="{x + 13}" cy="{y + 11}" r="3.5" fill="{dot}"{anim}/>'
        if dot
        else ""
    )
    return (
...
def featured(t: Theme, p: Project, visual: str) -> str:
    ...
    body = [
        "<defs>"
        + clip_card("clip", W, H, open_bottom=open_bottom)
        + "</defs>",
    ...
    body.append(
        f'<text x="{right}" y="58" class="eb" text-anchor="end">{esc(p.period)}</text>'
    )
    body.append(
        f'<text x="{x - 2}" y="104" class="ti" font-size="34">{esc(p.title)}</text>'
    )
    y = 138
    ...
    body.append("</g>")
    return document(
        W, H, "".join(body), "".join(css), title=p.title, desc=p.desc
    )
...
def tile(t: Theme, p: Project) -> str:
    ...
    css = [_base_css(t)]
    body = [
        "<defs>" + clip_card("clip", W, H) + "</defs>",
        card_frame(t, W, H),
        '<g clip-path="url(#clip)">',
    ]
    assert p.image, f"{p.slug} needs a photo for its tile"
    body.append(showcase(t, p.image, W, IMG_H))
    body.append(
        f'<line x1="0" y1="{IMG_H}" x2="{W}" y2="{IMG_H}" stroke="{t.border}"/>'
    )
    x, right = 28, W - 28
    body.append(
        f'<text x="{x}" y="{IMG_H + 38}" class="eb" font-size="11">{esc(p.eyebrow)}</text>'
    )
    body.append(
        f'<text x="{x - 1}" y="{IMG_H + 74}" class="ti" font-size="24">{esc(p.title)}</text>'
    )
    body.append(
        f'<text x="{right}" y="{IMG_H + 74}" class="eb" text-anchor="end">{esc(p.period)}</text>'
    )
    y = IMG_H + 104
    for line in wrap(p.desc, "sans", 400, 15, right - x)[:4]:
        body.append(
            f'<text x="{x}" y="{y}" class="de" font-size="15">{esc(line)}</text>'
        )
        y += 21
...
def all_cards(t: Theme) -> dict[str, str]:
    out = {
        f"project-{p.slug}": featured(t, p, visual) for p, visual in FEATURED
    }
